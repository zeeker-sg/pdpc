...
def fetch_data(existing_table) -> List[Dict[str, Any]]:
    ...
    with _make_client() as client:
        ...
        click.echo(f"Total guidance items: {total_items} across {total_pages} pages")
        ...
        for page in range(1, total_pages + 1):
            ...
            for item in items:
                ...
                click.echo(f"  -> {topic[:15]} | {title[:60]} | {published_date}")
    ...
    click.echo(f"\nDone: {len(results)} new guidance items.")
    return results
